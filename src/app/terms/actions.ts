...
export async function sendSupportTicket(data:SupportFormSchema ) {
  ...
  try {
    const { data } = await resend.emails.send({
      ...
      subject: `Wsparcie - złoszenie od ${name}`,
      html
    }) 

    if (!data) return { success: false, message: "Wystąpił problem z wysyłaniem zgłoszenia. Spróbuj ponownie" }
    return { success: true, message: "Dziękujemy za zgłosznie" }
  }catch (error) {
