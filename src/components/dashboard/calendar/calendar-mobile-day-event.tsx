...
export default function CalendarMobileDayEvent({event}:{event:CalendarAppointmentOverviewProps}) {
  //modal state
  ...
  return (
    <>
      <div
        ...
        className="absolute left-1/2 -translate-x-1/2 trans w-[99%] text-white text-sm rounded-md px-5 py-1.5 hover:cursor-pointer"
      ...
      <CalendarEventOverviewModal key={event.clientName} appointmentData={event} open={isOpen} onClose={() => {setIsOpen(false)}}/>
    </>
  );
}