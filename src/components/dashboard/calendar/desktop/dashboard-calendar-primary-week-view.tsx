...
export const DashboardCalendarPrimaryWeekView = () => {
  ...
  return (
    <div className="relative w-full h-full flex flex-col border-[0.5px] overflow-clip border-[#D4D4D4] shadow-lg rounded-2xl min-h-0">
      {/* CALENDAR NAVIGATION */}
      ...
      { /* DAY HEADINGS */}
      <div className="flex flex-row justify-evenly border-y border-y-[#D4D4D4]">  
        <div className="w-14"/> 
        ...
        <div className="h-fit w-full flex flex-col">
          <div className=" flex flex-row justify-evenly">
            {currentWeek.map((day, index) => (
              <div key={index} className="relative w-full flex flex-col border-l border-[#D4D4D4] first-of-type:border-l-0">
                {hours.map((_, i) => <div key={i} className="border-t border-[#D4D4D4] h-20"></div>)}
                {appointmentsForWeekIntervalData?.map((item, i)=> isSameDay(day, item.reservationStart) && 
                  <DashboardCalendarPrimaryWeekViewAppointment appointment={item} key={i}/>)
                }
              </div>
            ))}
          </div>
